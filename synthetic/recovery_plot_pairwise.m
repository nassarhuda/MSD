...
%%
recovery_plot_run(n,d,m,edgeps,vertps,ntrials,method)
...
parfor ei=1:numel(edgeps)
    ...
    for vi=1:numel(vertps)
        ...
        [row(vi),scorelist] = recovery_experiment_pairwise('mr',n,d,m,ntrials,vp,ep);
        scores{vi} = scorelist
    end
    Xpair(ei,:) = row;
    Mscores(ei,:) = scores;
...
%%
save pairwise_recovery.mat Xpair n d m edgeps vertps ntrials Mscores
